...
void minus(Matrix<T> const& input, U scalar,  Matrix<T>& output) {
#ifndef NDEBUG
...
template <typename T>
void minus(Matrix<T> const& left, Matrix<T> const& right, Matrix<T>& output) {
#ifndef NDEBUG
...
#endif
	for (size_t i = 0; i < output.getColumns() * output.getRows(); ++i) {
		output(i) = left(i) - right(i);
	}
}

template <typename T>
void minus(Vector<T> const& left, Vector<T> const& right, Vector<T>& output) {
#ifndef NDEBUG
